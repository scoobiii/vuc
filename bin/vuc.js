...
function printBanner() {
  console.log(`
┌─────────────────────────────────────────────────────────────┐
│  ⚡ VUC: Vortex Universal Connector & Governance Engine      │
...
Comandos Principais:
  vuc status                    Exibe diagnósticos do ambiente (Termux, Alpine, HW, RAM, Baseline)
  vuc baseline                  Gera auto-configuração de baseline dinâmica para este gadget/dev
  vuc adapters                  Lista os adaptadores registrados (Linux, Android, Windows, GitHub)
  vuc invoke <adapter> <action> Executa uma ação normatizada num adaptador com prova Ed25519
  vuc bench [--cloud]           Roda benchmark de desempenho local (e comparativo Cloud Run Free Tier)
  vuc gcloud <probe|bench|limits> Executa auditoria e benchmark de Google Cloud Free Tier
  vuc conformance               Roda bateria de conformidade nos 4 adaptadores (100% test suite)
  vuc llm [opções]              Executa prompt LLM governado (Qwen Coder local, Ollama ou Gemini)
  vuc bluesky <post|thread|...> Publica posts/threads no Bluesky com prova Ed25519 (AT Protocol)
  vuc mcp                       Inicia o servidor MCP local (JSON-RPC 2.0 via stdio) para Cursor, Claude, etc.
  vuc verify <proof.json>       Valida criptograficamente um ExecutionProof v1
  vuc mock <audit|fix>          Detecta e corrige automaticamente mocks forjados (Governança)
  vuc repo <subcomando> [repo]  Governança de repositório (inspect, bootstrap, verify, repair)

Subcomandos 'vuc repo':
  vuc repo inspect [repo]       Inspeciona governança, ruleset e conformidade sem alterar nada
  vuc repo bootstrap [repo]     Configura repositório no padrão VUA/Vortex (10/10 gates)
  vuc repo verify [repo]        Verifica status de conformidade e auditoria de drift
  vuc repo repair [repo]        Detecta drift e reaplica conformidade e rulesets

Opções do comando 'vuc llm':
...
Exemplos de Uso:
  ...
  vuc invoke android check_selinux
  vuc invoke linux check_sandbox

  # 2. Rodar benchmark de throughput e latência criptográfica:
  vuc bench --iterations 500

  # 3. Testar Qwen Coder 0.5B offline no Termux (com Ollama ou Llama.cpp):
  vuc llm --provider ollama --model qwen2.5-coder:0.5b --prompt "console.log('hello')"

  # 4. Testar Gemini com API Key:
  vuc llm --provider gemini --model gemini-3.8-flash --prompt "Explique VUA em 1 frase"
`);
}
...
async function handleInvoke() {
  ...
  if (!adapterId || !action) {
    console.error('❌ Erro: Especifique o adaptador e a ação. Exemplo: vuc invoke linux check_sandbox');
    process.exit(1);
  }
...
async function handleVerify() {
  ...
  if (!filePath) {
    console.error('❌ Erro: Forneça o caminho do arquivo JSON da prova. Ex: vuc verify proof.json');
    process.exit(1);
  }
...
async function handleBluesky() {
  ...
  if (subCmd === 'post') {
    if (!textArg) {
      console.error('❌ Erro: Informe o texto do post. Exemplo: vuc bluesky post "Olá Bluesky do VUA!"');
      process.exit(1);
    }
  ...
  if (subCmd === 'thread') {
    ...
    if (posts.length === 0) {
      console.error('❌ Erro: Informe os textos dos posts do thread. Exemplo: vuc bluesky thread "Post 1" "Post 2"');
      process.exit(1);
    }
...
async function handleMockDetector() {
  ...
  if (totalMocksFound === 0) {
    ...
    if (sub !== 'fix') {
      console.log(`Execute: 'vuc mock fix' para sanitizar e corrigir automaticamente.`);
    }
  }
  console.log(`═════════════════════════════════════════════════════════════\n`);
}
...
async function handleGcloud() {
  ...
  if (sub === 'limits' || sub === 'freetier') {
    ...
    console.log(`Subcomando desconhecido: ${sub}. Use: vuc gcloud <limits|probe|bench>`);
  }
}
