...
assert.equal(allowed.result?.capability_executed, true);
assert.equal(allowed.result?.execution_proof?.executed, true);
assert.equal(allowed.result?.execution_proof?.principal_id, principal);
assert.equal(allowed.result?.execution_proof?.proof_hash !== undefined, true);
assert.equal(verifyExecutionProof(allowed.result.execution_proof).valid, true);
...
assert.equal(crossTenant.result?.success, false);
assert.equal(crossTenant.result?.execution_proof?.executed, false);
assert.equal(crossTenant.result?.execution_proof?.status, 'POLICY_DENIED');
assert.match(String(crossTenant.result?.error?.message), /tenant/i);
...
assert.equal(escalated.result?.success, false);
assert.equal(escalated.result?.execution_proof?.executed, false);
assert.equal(escalated.result?.execution_proof?.status, 'POLICY_DENIED');
...
assert.equal(scopeEscalation.result?.success, false);
assert.equal(scopeEscalation.result?.execution_proof?.executed, false);
assert.equal(scopeEscalation.result?.execution_proof?.status, 'POLICY_DENIED');
assert.match(String(scopeEscalation.result?.error?.message), /scope/i);
